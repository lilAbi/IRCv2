...
void Logger::init_loggers() {
    ...
    auto thread_console_sink = std::make_shared<spdlog::sinks::stdout_color_sink_mt>();
    ...
    m_thread_logger    = std::make_shared<spdlog::logger>( "NetworkService", spdlog::sinks_init_list{thread_console_sink} );

    m_default_logger->set_level(spdlog::level::trace);
    m_default_logger->flush_on(spdlog::level::err);

    m_thread_logger->set_level(spdlog::level::trace);
    m_thread_logger->flush_on(spdlog::level::err);
}
